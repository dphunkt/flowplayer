...
package org.flowplayer.view {
	...
	public class Launcher extends StyleableSprite implements ErrorHandler {		
		...
		private function initPhase3(event:Event = null):void {
			...
			_config.getPlaylist().onBeforeBegin(function(event:ClipEvent):void { 			
				hideErrorMessage(); 
			});
...
//			addViewLiteners(_screen);
		...
		private function onMouseWheel(event:MouseEvent):void {
			_flowplayer.dispatchEvent(PlayerEvent.mouseWheel(event.delta));
		}
		...
		private function onStageVideoStateChange(event:ClipEvent):void {
			...
			if ( stageVideo ) {
				//var video:StageVideoWrapper = _flowplayer.streamProvider.getVideo(event.target as Clip) as StageVideoWrapper;
				log.debug("screen bounds" + _screen.getDisplayBounds());
				stageVideo.viewPort = _screen.getDisplayBounds();
                ...
                log.debug("screen position" + _screen.x + ' ' + _screen.y);
		...
		private function addPlayListListeners():void {
			...
			playlist.onBeforeBegin(onBeforeBegin);
			playlist.onError(onClipError);
            playlist.onBegin(onBegin);
            playlist.onStart(onStart);
		}
		
		//search for auth params in clip url and decrypt
		private function onBeforeBegin(event:ClipEvent):void {
			var clip:Clip = event.target as Clip;
			var playlist:Playlist = _config.getPlaylist();
			
			//click to MT if the player ain't hosted on our sites
//			var page:String = URLUtil.pageUrl;
//			if (page.indexOf('madthumbs.com') == -1 && page.indexOf('sublimedirectory.com') == -1 && page.indexOf('maxporn.com') == -1){ 
//				clip.linkUrl = 'http://www.madthumbs.com/';
//			}
			
			//short circuit player if clip is not from the MadThumbs CDN. Only for embed player.          
//			var newClip:Clip = _config.createClip({"url":"http://cache.tgpsitecentral.com/madthumbs/images/madthumbs_bumper.swf", "scaling": "fit", "linkUrl":"http://www.madthumbs.com", "duration":4, "autoPlay":false, "autoBuffering":true});
//			if(playList.commonClip.linkUrl == null || clip.linkUrl.indexOf('www.madthumbs.com') == -1 || playList.toString().indexOf('madthumbscdn.com') == -1){
//                playList.replaceClips(newClip);
//            }			
		}
        ...
        private function onStart(event:ClipEvent):void {
            if (!Clip(event.target).seekToOnStart) return;
			_flowplayer.seek(Clip(event.target).seekToOnStart);
        }
